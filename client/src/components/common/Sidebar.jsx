import { Bell, BookOpen, Building2, CalendarDays, ChartLine, ChevronLeft, Heart, LayoutDashboard, LibraryBig, LogOut, Settings, Users } from "lucide-react";
import { NavLink, useNavigate } from "react-router-dom";
import useAuth from "../../hooks/useAuth";

const navigation = {
  student: [
    ["Dashboard", "/student/dashboard", LayoutDashboard], ["Browse Books", "/books", BookOpen], ["Borrowed Books", "/my-books", LibraryBig], ["Wishlist", "/wishlist", Heart], ["Seat Booking", "/seat-booking", CalendarDays], ["Notifications", "/notifications", Bell], ["Profile", "/profile", Users],
  ],
  librarian: [
...
function Sidebar({ isOpen, onClose }) {
  ...
  return <>
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 flex w-[270px] flex-col border-r border-gray-200 bg-white pt-20 transition-transform duration-300 lg:static lg:z-0 lg:translate-x-0 lg:pt-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      {user?.role === "student" ? <button type="button" onClick={handleLogout} className="m-4 flex items-center gap-3 rounded-xl border border-[#F3E8C8] px-3 py-3 text-sm font-medium text-gray-700 transition hover:bg-amber-50"><LogOut size={19} />Logout</button> : <div className="m-4 rounded-xl bg-gray-50 p-4"><p className="text-sm font-semibold text-gray-800">Need help?</p><p className="mt-1 text-xs leading-relaxed text-gray-500">Find your next great read with BookHive.</p></div>}
    </aside>
  </>;
}
